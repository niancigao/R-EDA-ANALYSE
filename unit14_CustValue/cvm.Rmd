...
詳細的作法後面的課程會再討論，以下我們先以`R2`族群做示範。
...
# 顧客i預期營收增額 : 
# delta Ri = 回購機率*預期營收(工具前) - 回購機率*預期營收(工具後)
...
# Target$PredRevenue 的變化量 ?? (可能有誤)

```
結果顯示！這一項工具對`R2`顧客的平均報酬是負的，WHY?

##### 7.3 選擇行銷對象
但是，我們還是可以從`R2`挑出許多預期報酬很大的行銷對象
```{r}
