...
```{r}
pca = PCA(D[,1:10])   #要帶數值，grapf=F : 不畫圖(兩張)
...
# 1 "$eig" "特徵值"
# 2 "$var" "變量的結果"
# 3 "$var$coord" "各變數在各尺度的座標"
# 4 "$var$cor" "相關變量 - 維度"
# 5 "$var$cos2" "各變數在各尺度呈現的資訊比率"
# 6 "$var$contrib" "變量的貢獻"
# 7 "$ind" "個人結果"
# 8 "$ind$coord" "個體在各尺度的座標"
# 9 "$ind$cos2" "個體在各尺度呈現的資訊比率"
# 10 "$ind$contrib" "個人貢獻"
# 11 "$call" "匯總統計"
# 12 "$call$centre" "變量的平均值"
# 13 "$call$ecart.type" "變量的標準差"
# 14 "$call$row.w" "個人權重"
# 15 "$call$col.w" "變量的權重"
...
&emsp; § 原始尺度之間通常存在相關性($Cov(A,B)\neq0$)，所以它們所攜帶的資訊會有部分重疊<br>
